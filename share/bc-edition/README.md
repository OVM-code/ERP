# ERP Setup Assistant — Business Central Edition

A decision-support system for a **Microsoft Dynamics 365 Business Central** functional
consultant. It knows the setup options of Business Central and its add-ons, argues
which options fit a specific client and why, learns from every decision you record,
and runs on any LLM platform that can read files — Claude directly, or Microsoft
Copilot Studio via the included deployment kit.
...
> **How you received this:** this repo is a generated, read-only distribution of the
> Business Central knowledge layer. Updates arrive as new export commits — `git pull`
> to receive them. To use it as your own working system (adding your clients and
> lessons), **fork or copy it**; don't work directly in this repo, and never push
> client data back here.
...
1. **Check for relevant software updates first** (at most once a day per stack — see
   ...
   exact stack (standard BC knowledge + add-on overlays that change or extend it).
...
| [`clients/`](clients/) | One folder per client: intake + Setup Decision Records (empty here — yours to fill in your copy) |
| [`expertise/`](expertise/) | Lessons learned across clients — consulted before every recommendation (template only here) |
...
- **In Claude** (Code, desktop, or web with your copy of this repo attached): just start
  asking. `CLAUDE.md` loads the role automatically. The assistant reads and *writes*
  files — intakes, SDRs, lessons — directly.
...
⚠️ Your copy will contain client-related information — keep it private and put only
